...
measured_table = readtable("standing_and_sitting.csv");
kuka_table = readtable("sitting_and_standing_kuka.csv");
...
start_of_data = 167; % Eliminate the first part of the data
end_of_data = 107; % Eliminate ht elast part of the data
...
% the experiment
new_kuka_x = resample(kuka_data(:,1), 280, 1000);
new_kuka_y = resample(kuka_data(:,2), 280, 1000);
new_kuka_z = resample(kuka_data(:,3), 280, 1000);

% Eliminate data that should not be taken into consideration wfor
% comparison
start_of_data = 9; % Eliminate the first part of the data
end_of_data = 7; % Eliminate ht elast part of the data
