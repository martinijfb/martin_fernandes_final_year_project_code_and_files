...
% read the data
% testc.csv is sitting and standing
% testd.csv is walking motion
% testu.csv is underground

table_data = readtable("testd.csv");
...
start_of_data = 30; % Eliminate the first part of the data
end_of_data = 225; % Eliminate ht elast part of the data
...
subplot(2,2,1);
plot(final_data_2(:,2));
title('Horizontal Movement', 'FontSize',24);
ylabel('metres', 'FontSize',16);
xlabel('Number of data points', 'FontSize',16);
xlim([0 length(final_data_2(:,2))]);
grid on

subplot(2,2,2);
plot(final_data_2(:,3));
title('Distance to Aurora', 'FontSize',24);
ylabel('metres', 'FontSize',16);
xlabel('Number of data points', 'FontSize',16);
xlim([0 length(final_data_2(:,2))]);
grid on
...
subplot(2,2,3);
plot(final_data_2(:,4));
title('Vertical Movement', 'FontSize',24);
ylabel('metres', 'FontSize',16);
xlabel('Number of data points');
xlim([0 length(final_data_2(:,2))]);
grid on
...
%writematrix(final_data_2,'walking_straight_for_test.csv') 